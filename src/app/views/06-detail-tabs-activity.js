...
function renderDetailTab(j,tab){
  ...
  if(tab==='photos'){
    ...
    const photosHtml=filtered.map((p,i)=>{
      ...
      const url=p.url||p;
      const isVid=(p&&p.type==='video')||(typeof p==='object'&&/^video\//.test(p.mime||''));
      const media=isVid
        ? `<div class="photo-video" data-view-photo="${idx}">${p.poster?`<img src="${esc(p.poster)}" alt="" loading="lazy">`:`<video src="${url}#t=0.1" preload="metadata" muted playsinline></video>`}<span class="photo-play" aria-hidden="true">▶</span></div>`
        : `<img src="${url}" alt="" data-view-photo="${idx}" loading="lazy">`;
      return `<div class="photo-wrap${isVid?' is-video':''}">
    ...
    return `
      <div class="section">
        <div class="section-hd">Photos &amp; Video <span>${photos.length} total</span></div>
