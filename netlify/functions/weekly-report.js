// Weekly financial report — Netlify Scheduled Function (see AGENT_API_DESIGN.md).
//
// Runs on the schedule set in netlify.toml ([functions."weekly-report"]).
// It reads the company's data with the Firebase Admin SDK (reusing
// FIREBASE_SERVICE_ACCOUNT — no agent API key needed), asks Claude for a short
// narrative, emails the report, and stores a weekly snapshot so next week's
// summary can report real week-over-week changes.
//
// Env vars (set once in Netlify):
//   REPORT_TO          recipient email (required to send)
//   REPORT_COMPANY     company id to report on (default "wfs")
//   ANTHROPIC_API_KEY  enables the AI summary (optional; omit for numbers only)
//   SMTP_USER/SMTP_PASS SMTP creds (same as send-invoice); SMTP_HOST/PORT optional
//   REPORT_FROM        optional From address (defaults to SMTP_USER)

const nodemailer = require('nodemailer');
const { db } = require('./_lib/firebaseAdmin');
const { computeOverview, computeReportDetail } = require('./_lib/reports');
...
// Short narrative from Claude. Returns null if not configured or on any error —
// the report still sends with the numbers.
...
exports.handler = async () => {
  try {
    ...
    // Week-over-week: compare to the most recent stored snapshot.
    ...
    if (!to) {
      ...
      return { statusCode: 200, body: 'Report computed but REPORT_TO not set; no email sent.' };
    }

    const user = process.env.SMTP_USER, pass = process.env.SMTP_PASS;
    if (!user || !pass) {
      console.error('[weekly-report] SMTP_USER/SMTP_PASS not set — cannot email the report.');
      return { statusCode: 200, body: 'Report computed but SMTP not configured; no email sent.' };
    }
    ...
    await transporter.sendMail({
      ...
      to,
      subject: `Weekly financial report — ${company} — ${new Date().toISOString().slice(0, 10)}`,
      text,
      html,
    });
